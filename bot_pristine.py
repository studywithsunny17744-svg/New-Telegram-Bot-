...
async def earn_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    earn_msg = (
        ...
        f"💰 *EARN FREE COINS*\n"
        f"{SUB_LINE}\n\n"
        f"1�?⃣ Click the ad link below.\n"
        f"2�?⃣ Wait for *20 seconds* on the page.\n"
        f"3�?⃣ Come back and type `/claim`.\n\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_msg = (
        ...
        f"�?��? `/like` ➜ _Send like_ 🔒 _Admin only_\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        ...
        f"  �?��? `/like <uid> <region>` — Send like 🔒 _Admin only_\n"
        ...
        if isinstance(json_data, dict):
            if json_data.get("status") == "error" or json_data.get("error") is True:
                return (
                    ...
                    f"{SUB_LINE}\n\n"
                    f"⚠�? {json_data.get('message', 'Unknown error')}\n"
                    f"{FOOTER}"
                )
...
def format_ffinfo(data):
    ...
    msg += f"├─ Developer: ! Ｍ�?ｎｉ２７２\n"
    msg += f"└─ Signature: ! Ｍ�?ｎｉ２７２ — Always learning 💻 Full-stack Developer\n\n"
    msg += f"! Ｍ�?ｎｉ２７２ Intelligence • ! Ｍ�?ｎｉ２７２ | UID: {p.get('accountId', 'N/A')}•\n"
...
async def group_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    warn_msg = (
        f"⚠�? <b>Warning</b> <a href=\"tg://user?id={user.id}\">{name}</a>\n"
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "admin_panel":
        keyboard = [
            ...
            [InlineKeyboardButton("�?��? Reset Likes", callback_data="reset_likes_btn")],
            [InlineKeyboardButton("🔙 Back", callback_data="back_to_start"), InlineKeyboardButton("✖�? Close", callback_data="close_admin")]
        ]
        admin_msg = (
            f"{HEADER}"
